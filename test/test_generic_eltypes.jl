...
@testset "Generic eltypes: Float32" begin
    for A in _matrices(Float32)
        ...
        @test maximum(abs.(A * x .- b)) <= 1e-4
        xa = K' \ b
        @test eltype(xa) === Float32
        @test maximum(abs.(A' * xa .- b)) <= 1e-4
        # klu!
        B = copy(A); B.nzval .*= 2.0f0
        PureKLU.klu!(K, B)
        x = K \ b
        @test maximum(abs.(B * x .- b)) <= 1e-4
        ...
        @test maximum(abs.(Float64.(xrestored) .- x64)) <= 1e-4
    end
end
...
@testset "Generic eltypes: BigFloat" begin
    for A in _matrices(BigFloat)
        ...
        @test maximum(abs.(A * x .- b)) <= BigFloat(1e-50)
        xa = K' \ b
        @test eltype(xa) === BigFloat
        @test maximum(abs.(A' * xa .- b)) <= BigFloat(1e-50)
        # klu!
        B = copy(A); B.nzval .*= BigFloat(2)
        PureKLU.klu!(K, B)
        x = K \ b
        @test maximum(abs.(B * x .- b)) <= BigFloat(1e-50)
        ...
        Krestored = PureKLU.klu(A)
        xrestored = Krestored \ b
        @test maximum(abs.(Float64.(xrestored) .- x64)) <= 1e-12
    end
end
...
@testset "Generic eltypes: ComplexF32" begin
    for A in _matrices(ComplexF32)
        ...
        @test maximum(abs.(A * x .- b)) <= 1f-4
        xa = K' \ b
        @test eltype(xa) === ComplexF32
        @test maximum(abs.(A' * xa .- b)) <= 1f-4
        # klu!
        B = copy(A); B.nzval .*= ComplexF32(2)
        PureKLU.klu!(K, B)
        x = K \ b
        @test maximum(abs.(B * x .- b)) <= 1f-4
    end
end
...
@testset "Generic eltypes: ForwardDiff.Dual" begin
    ...
    for N in (1, 3)
        ...
        x = K \ b_dual
        @test eltype(x) === DT
        @test maximum(abs.(ForwardDiff.value.(Ad * x .- b_dual))) <= 1e-10
        ...
        @test maximum(abs.(ForwardDiff.value.(x) .- x0)) <= 1e-10
        # each partial matches finite-difference
        for j in 1:N
            ad_partial = [ForwardDiff.partials(x[i])[j] for i in 1:n]
            @test maximum(abs.(ad_partial .- fd_partials[:, j])) <= 1e-6
        end

        # adjoint solve also works with Dual
        xa = K' \ b_dual
        @test eltype(xa) === DT
        @test maximum(abs.(ForwardDiff.value.(Ad' * xa .- b_dual))) <= 1e-10
        ...
        PureKLU.klu!(K, Ad2)
        x2 = K \ b_dual
        @test maximum(abs.(ForwardDiff.value.(Ad2 * x2 .- b_dual))) <= 1e-10
    end
end
