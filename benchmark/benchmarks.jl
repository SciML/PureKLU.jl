...
let g = (SUITE["factor_only"] = BenchmarkGroup())
    for (name, A) in MATRICES
        ...
        g[name]["pureklu_fma"] = @benchmarkable PureKLU.klu_factor!(K) setup=(K = (begin
            K = PureKLU.KLUFactorization($A); K.common.use_fma = Val(true)
            PureKLU.klu_analyze!(K); K
        end))
        g[name]["pureklu_nofma"] = @benchmarkable PureKLU.klu_factor!(K) setup=(K = (begin
...
let g = (SUITE["solve"] = BenchmarkGroup())
    for (name, A) in MATRICES
        n = size(A, 1)
        Random.seed!(0xC0FFEE); b = randn(n)
        g[name] = BenchmarkGroup()
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for phase in ("analyze_factor", "analyze", "factor_only", "refactor", "solve", "tsolve")
        ...
        for (name, _) in MATRICES
            ...
            tref = minimum(ref).time / 1e3
            ton  = on  === nothing ? NaN : minimum(on).time / 1e3
            toff = off === nothing ? NaN : minimum(off).time / 1e3
