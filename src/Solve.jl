...
function _klu_solve_impl!(
    ...
    # factor would divide by the stored zero pivot and produce Inf/NaN, so we
    # leave the status singular for the caller to handle (`check=true` raises a
    # `SingularException`).  For a well-conditioned factor `numerical_rank ==
    # EMPTY`, so this is a no-op and the well-conditioned solve is unchanged.
    ...
    for col in 1:nrhs
        ...
        for block in nblocks:-1:1
            ...
            if nk == 1
                ...
                _klu_lsolve!(
                    nk, view(Lip, (k1 + 1):k2), view(Llen, (k1 + 1):k2),
                    Num.LUbx[block], view(X, (k1 + 1):k2), fma_val
                )
                _klu_usolve!(
                    nk, view(Uip, (k1 + 1):k2), view(Ulen, (k1 + 1):k2),
...
function _klu_lsolve!(
    ...
    return @inbounds for k in 0:(n - 1)
        xk = X[k + 1]
        lip = Int(Lip[k + 1])
        len = Int(Llen[k + 1])
...
function _klu_usolve!(
        n::Int, Uip::AbstractVector{Ti}, Ulen::AbstractVector{Ti},
        block::KLUNumericBlock{Tv, Ti}, Udiag::AbstractVector{Tv},
        X::AbstractVector{Tv}, fma_val::Val
    ) where {Tv, Ti}
...
function klu_tsolve!(
    ...
    return _klu_tsolve_impl!(
        Sym, Num, B, common, common.use_fma;
        conj_solve
    )
end

function _klu_tsolve_impl!(
        Sym::KLUSymbolic{Ti}, Num::KLUNumeric{Tv, Ti, Tr},
        B::AbstractVecOrMat{Tv}, common::KLUCommon{Ti},
        fma_val::Val;
        conj_solve::Bool = false
...
function klu_refactor!(
    ...
    # the inner loop).
    if scale > 0
        return _klu_refactor_impl!(
            Sym, Num, Ap, Ai, Ax, common, common.use_fma,
